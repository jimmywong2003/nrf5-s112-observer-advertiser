...
static void beacon_scanner_on_sys_evt(uint32_t event, void *p_context)
{
    uint32_t err_code;
    switch (event)
    {
        case NRF_EVT_RADIO_SESSION_IDLE:
            NRF_LOG_DEBUG("SESSION_IDLE");
...
NRF_SDH_SOC_OBSERVER(m_sys_obs, 0, beacon_scanner_on_sys_evt, NULL);


void app_beacon_scanner_on_ble_evt(ble_evt_t * p_ble_evt)
{
    //switch (p_ble_evt->header.evt_id)
    //NRF_LOG_INFO("on_ble_evt: %x", p_ble_evt->header.evt_id);
